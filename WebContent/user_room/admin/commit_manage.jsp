...
<body>
	...
	<%
		...
		allList = vrCommitDao.findByState("wait", vrCommitDao.findCount(), 1);
		List<VrCommit> list = new ArrayList<>();
		Iterator<VrCommit> iterator = allList.iterator();
		while(iterator.hasNext()){
			VrCommit c = iterator.next();
			if("userCommit".equals(c.getCommitType())){
				list.add(c);
			}
		}
		...
		int rawSize;//列数
		int colSize;//行数			
		String[] headers = { "留言ID", "留言内容", "留言时间", "留言者", "留言状态"};//表头
		...
		for (int raw = 0; raw < colSize; raw++) {
			...
			data[raw][1] = tmp.getCommitContext().substring(0, 5)+"...";
	...
	<div>
 	...
 	if (col == 0) {
 %> <a
						href="/Venue_Reservation/user_room/admin/commit_info.jsp?id=<%=list.get(raw).getCommitId()%>">
							<%=data[raw][col]%>
					</a> <%
 	} else {
 %> <%=data[raw][col]%> <%
